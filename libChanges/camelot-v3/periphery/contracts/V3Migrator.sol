...
import {TransferHelper as PeripheryTransferHelper} from './libraries/TransferHelper.sol';
...
contract V3Migrator is IV3Migrator, PeripheryImmutableState, PoolInitializer, Multicall, SelfPermit {
    ...
    function migrate(MigrateParams calldata params) external override {
        ...
        PeripheryTransferHelper.safeApprove(params.token0, nonfungiblePositionManager, amount0V2ToMigrate);
        PeripheryTransferHelper.safeApprove(params.token1, nonfungiblePositionManager, amount1V2ToMigrate);
        ...
        if (amount0V3 < amount0V2) {
            if (amount0V3 < amount0V2ToMigrate) {
                PeripheryTransferHelper.safeApprove(params.token0, nonfungiblePositionManager, 0);
            }
            ...
            if (params.refundAsNative && params.token0 == WNativeToken) {
                ...
                PeripheryTransferHelper.safeTransferNative(msg.sender, refund0);
            } else {
                PeripheryTransferHelper.safeTransfer(params.token0, msg.sender, refund0);
            }
        }

        if (amount1V3 < amount1V2) {
            if (amount1V3 < amount1V2ToMigrate) {
                PeripheryTransferHelper.safeApprove(params.token1, nonfungiblePositionManager, 0);
            }
            ...
            if (params.refundAsNative && params.token1 == WNativeToken) {
                ...
                PeripheryTransferHelper.safeTransferNative(msg.sender, refund1);
            } else {
                PeripheryTransferHelper.safeTransfer(params.token1, msg.sender, refund1);
            }
        }
    }
}
