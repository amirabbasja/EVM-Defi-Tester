...
import '@camelotV3_OZ/contracts/token/ERC20/IERC20.sol';
...
abstract contract PeripheryPayments is IPeripheryPayments, PeripheryImmutableState {
    ...
    function unwrapWNativeToken(uint256 amountMinimum, address recipient) external payable override {
        ...
        if (balanceWNativeToken > 0) {
            ...
            PeripheryTransferHelper.safeTransferNative(recipient, balanceWNativeToken);
        }
    }
    ...
    function refundNativeToken() external payable override {
        if (address(this).balance > 0) PeripheryTransferHelper.safeTransferNative(msg.sender, address(this).balance);
    }
