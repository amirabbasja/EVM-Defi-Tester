...
import {IUniswapV3Pool} from "../lib/sushiswap-v3-core/contracts/interfaces/IUniswapV3Pool.sol";
...
contract UniswapV3Tester is Test {
    ...
    ISwapRouter sushiV3SwapRouter = ISwapRouter(SUSHISWAP_V3_ROUTER);
    ...
    modifier withWethUsdtPoolDefault() {
        ...
        console.log(sushiV3Factory.getPool(WETH_MAINNET, USDT_MAINNET, fee));
    ...
    function setUp() public {
        ...
        SushiswapV3Deployer deployerSushiV3 = new SushiswapV3Deployer();
        deployerSushiV3.run();
    }

    function test_sushiV3FactoryDeployment() public view {
        // Check to see if there is code at the address
        assert(address(SUSHISWAP_V3_FACTORY).code.length > 0);

        // Check constructor deployment
        assertEq(sushiV3Factory.feeAmountTickSpacing(500), 10);
    }

    function test_NonfungiblePositionManagerDeployment() public view {
        // Check to see if there is code at the address
        assert(address(SUSHISWAP_V3_POSITION_MANAGER).code.length > 0);
    }
    ...
    function test_SwapTokensForTokens() public withWethUsdtPoolDefault {
        ...
        WETH.approve(address(sushiV3SwapRouter), type(uint256).max);
        ...
        sushiV3SwapRouter.exactInputSingle(params);
        vm.stopPrank();
